...
<%
String url = request.getContextPath()+"/api/apiChatController/nearbyList";
%>
...
<body>
	...
	$(function() {
		...
		$('#dive_chat_nearbyList_Form').form({
			url : '<%=url%>',
			onSubmit : function() {
			...
			success : function(result) {
				parent.$.messager.progress('close');
				$("#dive_chat_nearbyList_result").text(result);
			}
		});
	});
</script>
	...
	<div class="easyui-layout" data-options="fit:true">
		...
		<div data-options="region:'center'">
			<form id="dive_chat_nearbyList_Form" action="" method="post">
				<table align="center" width="90%" class="tablex">
					...
					<tr>
						<td align="right" style="width: 180px;"><label>longitude(当前所在经度)：</label></td>
						<td><input name="longitude" type="text" class="span2" value=""/>（小数点不得超过10位有效数字）</td>
					</tr>
					<tr>
						<td align="right" style="width: 180px;"><label>latitude(当前所在纬度)：</label></td>
					...
					<tr>
						<td colspan="2" align="center"><input type="button"
							value="提交" onclick="javascript:$('#dive_chat_nearbyList_Form').submit();" /></td>
					</tr>
				</table>
			</form>
			<label>结果：潜友列表结果集，查看基础数据接口</label>
			<div id="dive_chat_nearbyList_result"></div>
			<div>
