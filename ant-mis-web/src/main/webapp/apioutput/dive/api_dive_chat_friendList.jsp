...
<%
String url = request.getContextPath()+"/api/apiChatController/friendList";
%>
...
<body>
	...
	$(function() {
		...
		$('#dive_chat_friendList_Form').form({
			url : '<%=url%>',
			onSubmit : function() {
			...
			success : function(result) {
				parent.$.messager.progress('close');
				$("#dive_chat_friendList_result").text(result);
			}
		});
	});
</script>
	...
	<div class="easyui-layout" data-options="fit:true">
		...
		<div data-options="region:'center'">
			<form id="dive_chat_friendList_Form" action="" method="post">
				<table align="center" width="90%" class="tablex">
					...
					<tr>
						<td align="right" style="width: 180px;"><label>accountIds(环信好友账号集合)：</label></td>
						<td><input name="accountIds" type="text" class="span2" value="" />(多个以逗号拼接，如：id1,id2,...,id3)</td>
					</tr>

					<tr>
						<td colspan="2" align="center"><input type="button"
							value="提交" onclick="javascript:$('#dive_chat_friendList_Form').submit();" /></td>
					</tr>
				</table>
			</form>
			<label>结果：潜友列表结果集，查看基础数据接口</label>
			<div id="dive_chat_friendList_result"></div>
			<div>
