...
<html>
<head>
<title>DriverAccount管理</title>
<jsp:include page="../inc.jsp"></jsp:include>
<c:if test="${fn:contains(sessionInfo.resourceList, '/driverAccountController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/driverAccountController/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/driverAccountController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前数据？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/driverAccountController/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/driverAccountController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function downloadTable(){
	    ...
	    $('#downloadTable').form('submit', {
	        url:'${pageContext.request.contextPath}/driverAccountController/download',
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="display: none;">
						<tr>
							...
							<th>联系电话</th>
							<td>
								<input type="text" name="conactPhone" maxlength="32" class="span2"/>
							</td>
						</tr>	
						<tr>	
							<th>骑手类型</th>
