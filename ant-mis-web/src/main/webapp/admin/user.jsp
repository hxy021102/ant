				...
				formatter : function(value, row, index) {
					...
					if(row.id == '0' && '${sessionInfo.id}' != '0') {
						...
						if ($.canEditPwd) {
							...
							str += $.formatString('<input type="button" class="button gray" onclick="editPwdFun(\'{0}\');" value="修改密码"/>', row.id);
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed"
					...
					<tr>
						...
						<th>最后修改时间</th>
						<td colspan="3">
							<input class="span2" name="modifydatetimeStart" placeholder="点击选择时间"
								   onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})"
								   readonly="readonly" />至<input class="span2" name="modifydatetimeEnd"
