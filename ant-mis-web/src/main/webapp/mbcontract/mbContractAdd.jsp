...
<script type="text/javascript">
	$(function() {
	 parent.$.messager.progress('close');
		$('#form').form({
			url : '${pageContext.request.contextPath}/mbContractController/add',
			onSubmit : function() {
		...
		<form id="form" method="post" enctype="multipart/form-data">
			...
			<table class="table table-hover table-condensed">
				...
				<tr>
					...
					<td>
						<input class="easyui-validatebox span2" data-options="required:true" name="expiryDateStart" type="text" onclick="WdatePicker({dateFmt:'<%=TmbContract.FORMAT_EXPIRY_DATE_START%>'})"  maxlength="0" />
					</td>
				</tr>	
				<tr>
					<th><%=TmbContract.ALIAS_EXPIRY_DATE_END%></th>
					<td>
						<input class="easyui-validatebox span2" data-options="required:true" name="expiryDateEnd" type="text" onclick="WdatePicker({dateFmt:'<%=TmbContract.FORMAT_EXPIRY_DATE_END%>'})"  maxlength="0"/>
					</td>
