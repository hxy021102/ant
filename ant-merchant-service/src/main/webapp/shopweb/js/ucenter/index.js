        ...
        if(!shop || shop.auditStatus == 'AS03') {
            ...
            if(shop.auditStatus == 'AS01') {
                ...
                if($(this).hasClass('cash')) {
                    if($(this).find('.cashBalance').attr('cashBalance') < 0) return;
                    type = 4;
                }
                window.location.href = '../balance/index.html?type=' + type;
            }
        }
    });
    ...
    ajaxPostSync('api/apiUserController/get', {}, function(data){
        if(data.success) {
            ...
            if(user.cashBalance >= 0) {
                $('.cashBalance').attr('cashBalance', user.cashBalance).html('￥' + Util.fenToYuan(user.cashBalance));
                $('li.cash').addClass('ui-row-flex');
            }
            ...
            if(user.mbShop) {
                ...
                $('.shopAddress').html(user.mbShop.address);
