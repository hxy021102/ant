...
function transfer() {
    ...
    amount = $.trim($('#amount').val()).replace(/[,]/g, '');
    ...
    if(orderId) {
        ...
        ajaxPost('api/apiBalanceController/balanceRemit', {amount:amount*1000/10,remitter:remitter,remitterTimeStr:$('#remitterTime').val(),content:remark,bankCode:bankCode}, function(data){
            $.loading.close();
            if(data.success) {
                $.alert('提交成功！请耐心等待审核','系统提示', function(){
